...
export interface FitnessPlayhead {
  currentFrame: number;
...
export function useFitnessPhysics(
  ...
  const [state, setState] = useState<FitnessPlayhead>({
    currentFrame: 0,
    playheadProgress: 0,
    floatY: 0,
    tiltDeg: 0,
  ...
  useEffect(() => {
    ...
    const loop = () => {
      ...
      if (el) {
        ...
        ag.current.floatY = lerp(ag.current.floatY, clamp(-nv * 45, -45, 20), 0.06);
        ag.current.tiltDeg = lerp(ag.current.tiltDeg, clamp(-nv * 3, -3, 3), 0.06);
        ...
        ag.current.speedIntensity = lerp(ag.current.speedIntensity, Math.min(absNv * 1.5, 1), 0.08);
